...
export class DuffelProvider implements TravelProvider {
  ...
  async searchFlights(request: TravelRequest): Promise<FlightOffer[]> {
    console.log('[Duffel] Searching flights with request:', request)
    
    if (!request.origin || !request.destination || !request.departureDate) {
      console.error('[Duffel] Missing required fields:', { origin: request.origin, destination: request.destination, departureDate: request.departureDate })
      throw new ProviderError(this.name, 'INVALID_REQUEST', 'Origin, destination, and departure date are required')
    }
    ...
    const requestBody = {
      data: {
        slices: [{ origin: request.origin, destination: request.destination, departure_date: request.departureDate }].concat(request.returnDate ? [{ origin: request.destination, destination: request.origin, departure_date: request.returnDate }] : []),
        passengers: Array.from({ length: request.passengerCount ?? request.guests ?? 1 }, () => ({ type: 'adult' })),
        cabin_class: request.cabin ?? 'economy',
        max_connections: request.connectionLimit
      }
    }
    
    console.log('[Duffel] Request body:', JSON.stringify(requestBody, null, 2))
    
    try {
      const response = await fetchWithPolicy(`${travelConfig.duffel.baseUrl}/air/offer_requests`, { method: 'POST', headers: this.headers(), body: JSON.stringify(requestBody) }, travelConfig.timeoutMs, travelConfig.retries, this.name)
